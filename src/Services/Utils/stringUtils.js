...
const privateKeyUNETI = import.meta.env.VITE_UNETI_PRIVATE_SECRET
...
// Hàm mã hóa dữ liệu sử dụng AES với key
export const encryptAESWithKey = (plainText) => {
  return CryptoJS.AES.encrypt(plainText, privateKeyUNETI).toString()
}

// Hàm giải mã dữ liệu sử dụng AES với key
export const decryptAESWithKey = (cipherText) => {
  const bytes = CryptoJS.AES.decrypt(cipherText, privateKeyUNETI)
